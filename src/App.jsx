...
// Create a wrapper component that handles wallet connection changes
function DashboardContent({ activeView, navigateTo, isSidebarOpen, setSidebarOpen }) {
  const { connected } = useWallet();
  
  // Reset to overview when wallet disconnects
  useEffect(() => {
    if (!connected) {
      navigateTo('overview');
    }
      ...
      <div className="main-container">
        ...
        <aside className={`sidebar ${isSidebarOpen ? 'open' : ''}`}>
          <div className="sidebar-content">
            <nav className="sidebar-nav">
              ...
              <button 
                onClick={() => navigateTo('send-tokens')}
                className={`nav-button ${activeView === 'send-tokens' ? 'nav-button-active' : 'nav-button-inactive'}`}
                disabled={!connected}
              >
                <svg className="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <main className="main-content">
              ...
              <div className="content-section">
                ...
                <div className="card">
                  ...
                  <div className="feature-grid">
                    {/* Airdrop Card */}
                    <div 
                      className={`feature-card ${!connected ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                      onClick={() => connected && navigateTo('airdrop')}
                    >
                    ...
                    <div 
                      className={`feature-card ${!connected ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                      onClick={() => connected && navigateTo('send-tokens')}
                    >
                      <div className="feature-icon-container">
                        <div className="icon-bg">
                          <svg className="feature-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <h3 className="feature-title">Send SOL</h3>
                      </div>
                      <p className="feature-text">
                        Transfer SOL tokens from your wallet to another address on the Solana devnet.
                      </p>
                      <div className="feature-link">
                        <span className="feature-link-text">
                          Open
                    ...
                    <div 
                      className={`feature-card ${!connected ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                      onClick={() => connected && navigateTo('sign-verify')}
                    >
                      <div className="feature-icon-container">
                        <div className="icon-bg">
                          <svg className="feature-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          {(activeView === 'airdrop' && connected) ? (
            ...
            <div className="feature-container">
              <div className="card">
                <p className="text-center p-4">Please connect your wallet to use this feature</p>
              </div>
            </div>
          )}
          
          {/* Send Tokens View */}
          {(activeView === 'send-tokens' && connected) ? (
            <div className="feature-container">
              <div className="back-nav">
                <button 
                  onClick={() => navigateTo('overview')}
              ...
              <div className="card">
                <div className="feature-header">
                  ...
                  <h3 className="feature-header-title">Send SOL Tokens</h3>
                </div>
                <p className="feature-description">
                  Transfer SOL tokens from your connected wallet to any Solana address.
                </p>
          ...
          {/* Sign & Verify View */}
          {(activeView === 'sign-verify' && connected) ? (
            <div className="feature-container">
              <div className="back-nav">
                <button 
                  onClick={() => navigateTo('overview')}
          ...
          {/* Not Connected State */}
          {!connected && activeView !== 'overview' && (
            <div className="disconnected-state">
              <p>Please connect your wallet to access this feature</p>
              <WalletMultiButton className="wallet-multi-button mt-4" />
            </div>
          )}
        </main>
      </div>
